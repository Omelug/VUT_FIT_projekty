...
echo -n "Nepreskakuji se zastavky? " # OK 1
i=1
arrived=0
while IFS= read -r line; do
  action=$(echo "$line" | awk '{print $(NF-1)}')
  number=$(echo "$line" | awk '{print $NF}') #number or final
        #echo " $arrived $i $Z"
        if [[ $i -le $Z ]]; then
          if [[ $arrived == 0 ]]; then
            if [ "$action" == "to" ] && [ "$number" == $i ]; then
              arrived=1
            else
              echo "EXPECTED arrived to ${i}, got ${line}"
              glo_res="FAILED"
              break
            fi
          else
            if [ "$action" == "leaving" ] && [ "$number" == $i ]; then
              arrived=0
              ((i++))
            else
              echo "EXPECTED leaving ${i}, got ${line}"
              glo_res="FAILED"
              break
            fi
          fi
          ...
          if [[ $arrived == 0 ]]; then
            if [ "$action" == "to" ] && [ "$number" == "final" ]; then
              arrived=1
            else
              echo "EXPECTED arrived to final, got ${line}"
              glo_res="FAILED"
              break
            fi
          else
            if [ "$action" == "leaving" ] && [ "$number" == "final" ]; then
              arrived=0
              i=1
            else
              echo "EXPECTED leaving final, got ${line}"
              glo_res="FAILED"
              break
            fi
          fi
        fi
    done < <(grep -P "BUS: (arrived|leaving)" "${INPUT}")
if [ ${glo_res} == "OK" ]; then
    ((OK++))
fi
echo "${glo_res}"
